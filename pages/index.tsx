...
export default function TodayPage() {
  const [startDateISO, setStartDateISO] = useState<string>(() => {
    // Дата старта прикорма (пользователь задаёт один раз)
    const saved = localStorage.getItem("prikorm.startDateISO");
    return saved ?? todayISO();
  });

  useEffect(() => {
    localStorage.setItem("prikorm.startDateISO", startDateISO);
  }, [startDateISO]);
      ...
      <div className="mx-auto max-w-md px-4 pt-6">
        <div className={"rounded-3xl p-5 text-white " + (allDone ? "bg-emerald-600" : "bg-gray-900")}>
          ...
          <div className="mt-2 flex items-center gap-2">
            <Badge tone={allDone ? "ok" : "neutral"}>{doneCount}/{planDay?.feedings.length ?? 6} выполнено</Badge>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-sm font-bold text-gray-900">{f.time} <span className="ml-2">{cat.icon}</span></div>
        ...
        <div className="mt-6 flex gap-2">
          <SecondaryButton onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} className="w-full">Наверх</SecondaryButton>
          <SecondaryButton onClick={()=>alert("Дальше добавим экспорт/синхронизацию")} className="w-full">Экспорт</SecondaryButton>
        </div>
      </div>
